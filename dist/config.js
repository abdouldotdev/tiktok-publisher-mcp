...
export function readStore() {
    ...
    try {
        ...
        throw new Error(`Profile store at ${p} is not valid JSON (${err.message}). Fix or delete the file.`);
    }
}
...
export function updateProfile(name, updater) {
    const store = readStore();
...
export function required(value, field, profile) {
    if (value === undefined || value === null || value === "") {
        throw new Error(`Missing required configuration '${field}' for profile '${profile}'. Set it using tiktok_profile_set.`);
    }
    return value;
}
