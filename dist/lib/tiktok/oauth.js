...
import { getProfile, homeDir, required, updateProfile, DEFAULT_REDIRECT_URI, } from "../../config.js";
...
export function authStart(opts) {
...
export async function authComplete(opts) {
    let code = opts.code;
    ...
    if (opts.redirectUrl) {
        ...
        if (err) {
            throw new Error(`TikTok returned an error on redirect: ${err} — ${parsed.searchParams.get("error_description") ?? "no description"}`);
        }
        code = parsed.searchParams.get("code") ?? code;
        state = parsed.searchParams.get("state") ?? state;
    }
    if (!code)
        throw new Error("No authorization code — provide redirectUrl or code");
    code = decodeURIComponent(code);
    const all = readPending();
    const pending = state ? all[state] : undefined;
    if (state && !pending) {
        throw new Error(`Unknown state "${state}" — not issued by tiktok_auth_start on this machine, or pending record was cleared.`);
    }
    const profileName = opts.profile ?? pending?.profile;
    const account = opts.account ?? pending?.account;
    if (!profileName)
        throw new Error("Cannot resolve profile — pass profile explicitly");
    if (!account)
        throw new Error("Cannot resolve account name — pass account explicitly");
    ...
    if (res.error || !res.access_token) {
        throw new Error(`Token exchange failed: ${res.error ?? "no access_token"} — ${res.error_description ?? ""}`);
    }
    const stored = {
...
export function saveAccount(profileName, account, data) {
...
export function listAccounts(profileName) {
...
export function removeAccount(profileName, account) {
    const accounts = { ...(getProfile(profileName).accounts ?? {}) };
...
export async function refreshAccount(profileName, account) {
    const profile = getProfile(profileName);
    const current = profile.accounts?.[account];
    if (!current)
        throw new Error(`Account "${account}" not found in profile "${profileName}"`);
    ...
    if (res.error || !res.access_token) {
        throw new Error(`Refresh failed for "${account}": ${res.error ?? "no access_token"} — ${res.error_description ?? ""}. Re-run tiktok_auth_start.`);
    }
    const updated = {
        ...current,
        accessToken: res.access_token,
...
export async function getAccessToken(profileName, account) {
    const current = getProfile(profileName).accounts?.[account];
    if (!current) {
        throw new Error(`Account "${account}" not found in profile "${profileName}". Run tiktok_auth_start first.`);
    }
    const expiringSoon = !!current.expiresAt && new Date(current.expiresAt).getTime() < Date.now() + 60_000;
    if (expiringSoon)
        return (await refreshAccount(profileName, account)).accessToken;
