...
import { authComplete, authStart, listAccounts, refreshAccount, removeAccount, } from "../lib/tiktok/oauth.js";
export function registerAuthTools(server) {
    server.tool("tiktok_auth_start", "Begin TikTok OAuth flow. Returns an authorization URL to open in a browser. TikTok will redirect back to redirectUri.", {
            ...
            content: [
                {
                    type: "text",
                    text: JSON.stringify({
                        instructions: "Open authorizeUrl in browser, approve permissions, then copy the URL you are redirected to and pass it to tiktok_auth_complete.",
                        ...res,
                    }, null, 2),
                },
            ],
    ...
    server.tool("tiktok_auth_complete", "Complete TikTok OAuth flow by providing the redirected URL (or code and state). Exchanges code for tokens and saves account.", {
    ...
    server.tool("tiktok_accounts_list", "List all connected TikTok accounts for a given profile.", {
    ...
    server.tool("tiktok_account_refresh", "Manually refresh access tokens for a specific TikTok account.", {
        profile: z.string().default(DEFAULT_PROFILE).describe("Profile name."),
    ...
    server.tool("tiktok_account_remove", "Disconnect and remove a TikTok account from a profile.", {
        profile: z.string().default(DEFAULT_PROFILE).describe("Profile name."),
