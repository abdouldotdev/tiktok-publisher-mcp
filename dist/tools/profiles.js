import { z } from "zod";
import { DEFAULT_PROFILE, DEFAULT_REDIRECT_URI, deleteProfile, getProfile, listProfiles, updateProfile, } from "../config.js";
export function registerProfileTools(server) {
    server.tool("tiktok_profile_set", "Configure credentials and settings for a project profile (e.g. 'glowe', 'faithlock', 'autoviral').", {
    ...
    server.tool("tiktok_profile_get", "View configuration for a profile with secrets redacted.", {
    ...
    server.tool("tiktok_profile_list", "List all configured profiles in the TikTok farm.", {}, async () => {
        const profiles = listProfiles();
        return {
            content: [{ type: "text", text: JSON.stringify({ profiles }, null, 2) }],
        };
    });
    server.tool("tiktok_profile_delete", "Delete a profile and all its stored accounts.", {
