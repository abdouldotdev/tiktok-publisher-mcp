...
export function registerStatsTools(server) {
    server.tool("tiktok_creator_info", "Query creator permissions and publishing capabilities for a connected account.", {
    ...
    server.tool("tiktok_account_stats", "Fetch official profile metrics for a connected account (followers, following, likes, video count).", {
        profile: z.string().default(DEFAULT_PROFILE).describe("Profile name."),
        account: z.string().describe("Target TikTok account name."),
    }, async ({ profile, account }) => {
    ...
    server.tool("tiktok_post_stats", "Fetch official performance stats (views, likes, comments, shares) for specific post IDs.", {
        profile: z.string().default(DEFAULT_PROFILE).describe("Profile name."),
        account: z.string().describe("Target TikTok account name."),
    ...
    server.tool("tiktok_posts_list", "List recent posts and their analytics for the connected TikTok account.", {
        profile: z.string().default(DEFAULT_PROFILE).describe("Profile name."),
        account: z.string().describe("Target TikTok account name."),
